  ...
  try {
    ...
    if (!req.body.Merchant_uid) {
      return res.status(401).send({
        message: '주문 번호가 지급되지 않았습니다 주문 번호를 넘겨주세요',
      })
    }
    if (!req.body.imp_uid) {
      return res.status(402).send({
        message: 'uniqueKey가 지급되지 않았습니다. uniqueKey를 넘겨주세요',
      })
    }
    if (!req.body.price) {
      return res.status(403).send({
        message: '가격 정보가 지급되지 않았습니다. 가격 정보를 넘겨주세요',
      })
    }
    if (!req.body.amount) {
      return res.status(406).send({
        message: '구매갯수 정보가 지급되지 않았습니다. 가격 정보를 넘겨주세요',
      })
    }
