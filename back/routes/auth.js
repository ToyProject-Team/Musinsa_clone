    ...
    try {
        ...
        client.get(phoneNumber, async function (err, clientCheck) {
            ...
            if (!clientCheck) {
                ...
                return res
                    .status(400)
                    .send({ message: '인증 시간이 만료됐습니다' });
            }
            if (code != clientCheck) {
                return res
                    .status(401)
                    .send({ message: '인증 번호가 틀리셨습니다' });
            }
...
router.post('/findPassword', async (req, res, next) => {
    ...
    if (!exUser) {
        return res.status(401).send({
            message: '해당 로그인 아이디에 대한 유저 조회 결과가 없습니다',
        });
    }
    ...
    if (exUser.email != req.body.email && req.body.email != undefined) {
        return res.status(405).send({
            message: '로그인 아이디로 조회된 유저에 대한 이메일이 아닙니다',
        });
    }
    if (
...
router.post('/changePassword', async (req, res, next) => {
    try {
        ...
        const tokenId = await getAsync(
            req.headers.changepasswordtoken
                ? req.headers.changepasswordtoken
                : -111,
        );
        if (!tokenId) {
            return res.status(400).send({
                message: '토큰을 입력하지 않거나 인증 번호가 만료되었습니다',
            });
        }
...
router.post('/findId', async (req, res, next) => {
    try {
        ...
        if (!exUser && checkEmail) {
            res.status(402).send({
                message: '해당 이메일을 가진 유저의 조회 결과가 없습니다',
            });
        }
...
router.post('/isExistedLoginId', async (req, res, next) => {
    try {
        ...
        if (!exUser) {
            return res.status(401).send({
                message: '해당 아이디에 대한 유저 조회 결과가 없습니다',
            });
        }
...
router.post('/checkIsLoginIdCheckUser', (req, res, next) => {
    try {
        ...
        if (!userData) {
            return res.status(401).send({
                message:
                    '토큰을 복호화한 결과가 없습니다 입력을 다시 확인해주세요',
            });
        }
