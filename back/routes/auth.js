...
router.post('/signup', async (req, res, next) => {
  try {
    ...
    const checkSMS = await getAsync(
      req.headers.phonecheck ? req.headers.phonecheck : -111
    )
    const checkEmail = await getAsync(
      req.headers.emailcheck ? req.headers.emailcheck : -111
    )
    const checkSocialEmail = await getAsync(
      req.headers.encryptioncode ? req.headers.encryptioncode : -111
    )
    if (!checkSMS && !checkEmail) {
      return res.status(400).send({
        message: '이메일 인증 또는 휴대폰 인증이 완료되지 않은 사용자입니다',
      })
    }
    ...
    if (checkSMS) {
      if (req.body.phoneNumber !== checkSMS) {
        return res
          .status(403)
          .send({ message: '휴대폰 인증이 완료된 번호를 보내야합니다' })
      }
    } else {
      if (req.body.email !== checkEmail) {
        return res
          .status(405)
          .send({ message: '이메일 인증이 완료된 이메일을 보내야합니다' })
      }
    }
...
router.post('/checkEmail', async (req, res, next) => {
  try {
    ...
    if (!checkEmail) {
      return res
        .status(400)
        .send({ message: '이메일 인증 번호를 요청해야합니다' })
    }
    if (checkEmail != req.body.number) {
      return res
        .status(401)
        .send({ message: '이메일 인증 번호가 일치하지 않습니다' })
    }
    console.log('?')
    let emailCheck = CryptoJS.AES.encrypt(
...
router.post('/checkSMS', async (req, res, next) => {
  try {
    ...
    client.get(phoneNumber, async function (err, clientCheck) {
      ...
      phoneCheck = CryptoJS.AES.encrypt(
...
router.post('/findPassword', async (req, res, next) => {
  if (!req.body.loginId) {
    return res
      .status(400)
      .send({ message: '로그인 아이디가 전달되지 않았습니다' })
  }
  ...
  if (!exUser) {
    return res
      .status(401)
      .send({ message: '해당 로그인 아이디에 대한 유저 조회 결과가 없습니다' })
  }
  // console.log(req.headers)
  const client = redisClient
  const getAsync = promisify(client.get).bind(client)
  const userInfo = await getAsync(
    req.headers.phonecheck
      ? req.headers.phonecheck
      : req.headers.emailcheck
      ? req.headers.emailcheck
      : -111
  )
  ...
  if (!userInfo) {
    return res.status(402).send({
      message:
        'SMS 인증 시도하셔야합니다. 시도하셨다면 세션 스토리지의 phoneCheck가 headers로 전달됐는지 확인해주세요',
    })
  }
  ...
  if (exUser.email != req.body.email && req.body.email != undefined) {
    return res
      .status(405)
      .send({ message: '로그인 아이디로 조회된 유저에 대한 이메일이 아닙니다' })
  }
  if (
    exUser.phoneNumber != req.body.phoneNumber &&
    req.body.phoneNumber != undefined
  ) {
    return res.status(403).send({
      message: '로그인 아이디로 조회된 유저에 대한 전화번호가 아닙니다',
    })
  }
...
router.post('/changePassword', async (req, res, next) => {
  try {
    ...
    const getAsync = promisify(client.get).bind(client)
    const tokenId = await getAsync(
      req.headers.changepasswordtoken ? req.headers.changepasswordtoken : -111
    )
    if (!tokenId) {
      return res
        .status(400)
        .send({ message: '토큰을 입력하지 않거나 인증 번호가 만료되었습니다' })
    }
...
router.post('/findId', async (req, res, next) => {
  try {
    ...
    if (!checkSMS && !checkEmail) {
      return res.status(400).send({
        message: '인증번호를 입력하지 않거나 인증 번호가 만료되었습니다',
      })
    }
    const exUser = await User.findOne({
      where: {
        [Op.or]: [{ email: checkEmail }, { phoneNumber: checkSMS }],
      },
      attributes: ['loginId'],
    })
    ...
    if (!exUser && checkSMS) {
      res.status(401).send({
        message: '해당 휴대폰 번호를 가진 유저의 조회 결과가 없습니다',
      })
    }
    ...
    if (!exUser && checkEmail) {
      res
        .status(402)
        .send({ message: '해당 이메일을 가진 유저의 조회 결과가 없습니다' })
    }
...
router.post('/isExistedLoginId', async (req, res, next) => {
  try {
    if (!req.body.loginId) {
      return res
        .status(400)
        .send({ message: '로그인 아이디가 지급되지 않았습니다' })
    }
    const exUser = await User.findOne({
      where: {
        loginId: req.body.loginId,
    ...
    if (!exUser) {
      return res
        .status(401)
        .send({ message: '해당 아이디에 대한 유저 조회 결과가 없습니다' })
    }

    const userData = exUser.email ? exUser.email : exUser.phoneNumber
    loginIdCheckToken = await CryptoJS.AES.encrypt(
...
router.post('/checkIsLoginIdCheckUser', (req, res, next) => {
  try {
    ...
    if (!req.headers.loginidchecktoken) {
      return res
        .status(400)
        .send({ message: '헤더로 토큰이 지급되지 않았습니다' })
    }
    var bytes = CryptoJS.AES.decrypt(
      req.headers.loginidchecktoken,
      'secret key 123'
    )
    var userData = JSON.parse(bytes.toString(CryptoJS.enc.Utf8))
    if (!userData) {
      return res.status(401).send({
        message: '토큰을 복호화한 결과가 없습니다 입력을 다시 확인해주세요',
      })
    }
